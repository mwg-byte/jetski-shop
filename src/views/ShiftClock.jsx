...
import { Card, Row, Select, SectionTitle, btn, btnSm, LiveDot } from "../lib/ui";
...
export default function ShiftClock({ crew, onBack }) {
  ...
  async function clockOut(shift) {
    ...
    // guard rail: end any running job timers for this tech and log them
        ...
        {active.map((s) => {
          ...
          return (
            <Row key={s.id} style={{ marginBottom: 8 }}>
              ...
              {(mgr || s.tech_id === profile.id) && <button onClick={() => clockOut(s)} style={{ ...btnSm(C.orange), marginLeft: "auto" }}>Clock out</button>}
            </Row>
          );
        })}
        <Row>
        ...
        <div style={{ borderRadius: 6, overflow: "hidden", border: `1px solid ${C.line}` }}>
          {recent.map((s) => (
            <Row key={s.id} style={{ padding: "8px 12px", fontSize: 14, borderBottom: `1px solid ${C.line}`, fontFamily: BODY }}>
              ...
              {mgr && <button onClick={async () => { setShifts(shifts.filter((x) => x.id !== s.id)); await supabase.from("shifts").delete().eq("id", s.id); }} style={{ marginLeft: "auto", fontSize: 12, color: C.red }}>remove</button>}
            </Row>
          ))}
        </div>
      )}
